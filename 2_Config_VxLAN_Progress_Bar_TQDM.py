...
from tqdm.notebook import tqdm_notebook # color version of progress bar
import time
...
def config_VxLAN_Nexus_CP (task, pbar):
    ...
    pbar.update () # Update prgress bar once finish VxLAN configuration

# we are creating progress bar for total devices we have in our inventory

# tqdm is used to create simple progress bar
with tqdm (total=len(nr.inventory.hosts), desc = 'Configuration Progress', colour='yellow') as pbar:
    results = nr.run (task=config_VxLAN_Nexus_CP, pbar=pbar)
